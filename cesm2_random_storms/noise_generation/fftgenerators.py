...
#         in the spectral domain and transformed back to spatial domain after the
...
#     if domain == "spatial": # if domain is spatial
#         N = randstate.randn(input_shape[0], input_shape[1]) # generate field from the standard normal distribution
#     else: # else domain is spectral
#         if use_full_fft:
#             size = (input_shape[0], input_shape[1])
#         else:
#             size = (input_shape[0], int(input_shape[1] / 2) + 1)
#         theta = randstate.uniform(low=0.0, high=2.0 * np.pi, size=size) # generate random phase from (0 to 2pi)
#         if input_shape[0] % 2 == 0:
#             theta[int(input_shape[0] / 2) + 1 :, 0] = -theta[1 : int(input_shape[0] / 2), 0][::-1] # first column, half row become negative?
#         else:
#             theta[int(input_shape[0] / 2) + 1 :, 0] = -theta[1 : int(input_shape[0] / 2) + 1, 0][::-1]
#         N = np.cos(theta) + 1.0j * np.sin(theta) # compute exp(-i*theta) = cos(theta) - i * sin(theta)
#
#     # apply the global Fourier filter to impose a correlation structure
#     # if domain == "spatial":
#     if use_full_fft:
#         fN = fft.fft2(N) # compute the fft of white noise
#     else:
#         fN = fft.rfft2(N)
#     # else: # if domain is spectral
#     #     fN = N
#     fN *= F # fn * F
#     # if domain == "spatial":
#     if use_full_fft:
#         N = np.array(fft.ifft2(fN).real) # inverse fft and get the real part
#     else:
#         N = np.array(fft.irfft2(fN))
#     N = (N - N.mean()) / N.std() # normalize the field
#     # else: # if it is spectral
#     #     N = fN
#     #     N[0, 0] = 0.0
#     #     N /= spectral.std(N, input_shape, use_full_fft=use_full_fft) # N = N / std, not clear here...
#
#     return N
